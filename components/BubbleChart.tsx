...
const BubbleChart: React.FC<BubbleChartProps> = ({ data, maxBubbleSize = 160, width = 800, height = 600 }) => {
  ...
  return (
    <svg ref={svgRef} width={width} height={height} className="bg-white rounded-lg">
      ...
      <defs>
        ...
        <radialGradient id="bubbleGradient" cx="0.5" cy="0.5" r="0.5" fx="0.25" fy="0.25">
      ...
      {/* Render each bubble */}
      {nodes.map((node, i) => (
        <g key={i} transform={`translate(${node.x}, ${node.y})`} style={{ transition: "transform 0.1s ease-out" }}>
          {/* Main bubble */}
          <circle
            r={node.radius}
          ...
          {/* Count text */}
          <text textAnchor="middle" dy=".3em" fill="#fff" fontSize="12" fontWeight="bold">
            {node.data.count}
          </text>

          {/* Label text */}
          <text y={node.radius + 16} textAnchor="middle" fill="#333" fontSize="12">
            {node.data.label}
          </text>
        </g>
      ))}
